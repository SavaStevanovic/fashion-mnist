...
def get_model():
    ...
    return tf.keras.Model(inputs = inputLayer, outputs = output, name = 'wide_resnet_model')
...
def residual_block(net, filters):
    if net.shape[-1].value != filters:
        ...
        net_pre = net
    net = tf.keras.layers.Conv2D(filters=filters, kernel_size=3, padding='SAME', activation = 'relu')(net)
    net = tf.keras.layers.Dropout(rate = 0.3, seed = 42)(net)
    net = tf.keras.layers.Conv2D(filters=filters, kernel_size=3, padding='SAME', activation = 'relu')(net)
